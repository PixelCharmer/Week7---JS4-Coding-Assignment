...
let minusAge = ages[ages.length - 1] - ages[0] 
 //!to find the last element in the array - find the length and subtract 1. Sqaure brackets identify the elements index
...
for (let i = 0; i < ages.length; i++) {   
    //! this will iterate through all the elements of the array
...
for (let i = 0; i < names.length; i++) {    //! for loop to iterate through each element
    namesLengths.push(names[i].length);     
        //! uses the .push method to push the length of each name into the empty array that was previously declared
}
...
function multiplyWord(word, n) {   //! declares the function that takes 2 params "word" and "n"
    let concat = word.repeat(n);  
        //! uses the .repeat method that repeats "word" n number of times
    console.log(concat)
}
...
let concatedName = fullName('Kari', 'Alcoset');    
    //! calls the function and passes in values for the params
...
function sumNumbersArray(array) {      //! declares a function that takes 1 param
    ...
    if (total > 100) {             //! conditional statement that outputs true if total is > 100
        console.log(`The total = ${total} which is greater then 100 - True`);
...
sumNumbersArray(numbers1);   //! calls the function and passes in a param
...
function twoAverages(array1, array2) {    //! declares a function with 2 array params 
    ...
    for (const number of array1) {    
        //! creates a for of loop that declares a variable "number" and each element of array 1 will be referred to as number
...
function willBuyDrink(isHotOutside, moneyInPocket) {        
    //! declares a function with 2 params
    console.log("Params: ", isHotOutside, moneyInPocket);
    let buyDrink = isHotOutside === true && moneyInPocket > 10.5;  
        //! creates a varaible that uses conditionals for the value / one has to be strictly true AND has to be more then a specific amount of money for the function to return true 
